...
Usage:
  python create_compressed_col_external.py [--compressor COMP] [--accuracy ACC] [--filename DIR]
"""
import argparse
import array
import json
...
def run(DATACOL=DATACOL,FILENAME=FILENAME,
        ...
        DELETEOLD=DELETEOLD,SKIPDATA=SKIPDATA)-> tuple:
  ...
  for n in range(steps):
    ...
    # Better to flag NaN externally before this step
  ...
  if SKIPDATA==False:
      ...
      a_on_disk_size = get_size(f'{outname}/table.f{a_seq}.bp')
...
if __name__ == "__main__":  
  ...
  parser.add_argument("--skipdata", action='store_true', help="(False) If false make reduced accuray column in the MS (COPY_DATA)")
  ...
  run(DATACOL=DATACOL,FILENAME=FILENAME,
      STEPS=STEPS,ACCURACY=ACCURACY,COMPRESSOR=COMPRESSOR,
      DELETEOLD=DELETEOLD,SKIPDATA=SKIPDATA)
  sys.exit()
