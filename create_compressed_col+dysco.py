...
def make_DYSCO_column(DIRNAME, vis, start_rw=0, bitcount=8):
  """
  ...
  The given DIRNAME, if a string, will be opened and the file pointer returned
  If not it will be just written to, with vis
  The start_rw controls where this vis is written to
  """

  # if DIRNAME = string (else assume it is an open tab)
  if DIRNAME.astype=='str':
      tab = table(DIRNAME, readonly=False)
      coldesc = makearrcoldesc("COPY_DYSCO", np.complex64(0+0j), shape=ORIG_SHAPE[1:], options=1,
  ...
  tab.putcol('COPY_DYSCO',value=vis, startrow=start_rw)
  return(tab)
